// src/components/DeathTreemap.tsx
...
const DeathTree: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch("https://localhost:7268/odata/Death?$apply=groupby%28%28ProvinceState,CountryRegion,Lat,Long%29,aggregate%28Cases%20with%20sum%20as%20TotalDeath%29%29");
  ...
  const totalDeath = useMemo(
    () => data.reduce((acc, d) => acc + d.TotalDeath, 0),
    [data]
  );
  ...
  const treemapData: TreemapNode[] = useMemo(() => {
    return data
      .map((d, i) => ({
        ...
        size: d.TotalDeath,
        percentage: (d.TotalDeath / totalDeath) * 100,
        color: getColor(i),
      }))
      .sort((a, b) => b.size - a.size);
  }, [data, totalDeath]);

  if (loading) return <Layout>Loading Treemap...</Layout>;

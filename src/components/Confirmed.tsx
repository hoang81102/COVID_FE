...
const Confirmed: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          "https://localhost:7268/odata/Confirmed?$apply=groupby((ProvinceState,CountryRegion,Lat,Long),aggregate(Cases with sum as TotalConfirmed))"
        );
  ...
  const countryData = useMemo(() => {
    const map = new Map<string, { TotalConfirmed: number; Lat: number; Long: number }>();
    data.forEach(d => {
      if (map.has(d.CountryRegion)) {
        const existing = map.get(d.CountryRegion)!;
        map.set(d.CountryRegion, {
          TotalConfirmed: existing.TotalConfirmed + d.TotalConfirmed,
          Lat: (existing.Lat + d.Lat) / 2,
          Long: (existing.Long + d.Long) / 2,
        });
      } else {
        map.set(d.CountryRegion, { TotalConfirmed: d.TotalConfirmed, Lat: d.Lat, Long: d.Long });
      }
    });
    return Array.from(map.entries()).map(([CountryRegion, v]) => ({ CountryRegion, ...v }));
  }, [data]);

  const maxConfirmed = useMemo(() => Math.max(...countryData.map(d => d.TotalConfirmed), 1), [countryData]);

  const colorScale: ScaleSequential<string> = useMemo(
    () => scaleSequential<string>(interpolateRgb("#fee5d9", "#a50f15")).domain([0, maxConfirmed]),
    [maxConfirmed]
  );
  ...
  if (loading) return <Layout><div className="text-center mt-10 text-lg">Loading map data...</div></Layout>;
        ...
        <MapContainer center={[20, 0]} zoom={2} style={{ height: "100%", width: "100%" }} scrollWheelZoom>
          ...
          {countryData.map((item, idx) => (
            <CircleMarker
              ...
              radius={radiusFor(item.TotalConfirmed)}
              pathOptions={{
                fillColor: colorScale(item.TotalConfirmed),
                fillOpacity: 0.85,
